...
def knn_predict(X_train,y_train,X_test,k=5):
    ...
    for test_sample in X_test:
        #distances = [utils.euclidean_distance(test_sample,x_train) for x_train in X_train]
        #distances = [utils.manhattan_distance(test_sample,x_train) for x_train in X_train]
        distances = [utils.minkowski_distance(test_sample,x_train,5) for x_train in X_train]
        k_indices = np.argsort(distances)[:k] #preiau primele k valori din lista de distante
